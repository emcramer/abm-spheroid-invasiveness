...
hypo1_physicell_output_directories = "../PhysiCell/output/hypothesis1_random_seeds/"
hypo1_out_dirs = sorted(
    [d for d in os.listdir(hypo1_physicell_output_directories) if os.path.isdir(hypo1_physicell_output_directories+d)]
)

# only run if the results file needs to be re-generated
hypo1_simulation_results = []
hypo1_sim_props = {}
for j, d in enumerate(tqdm(hypo1_out_dirs)):
    print(f"Loading simulation: {d}")
    selected_steps = utils.load_simulation_by_interval("../PhysiCell/output/hypothesis1_random_seeds/"+d+"/")
    ...
    hypo1_simulation_results.append(props_df)
    print("Finished processing simulation.")
    
hypo1_simulation_results_df = pd.concat(hypo1_simulation_results)
hypo1_simulation_results_df['hours'] = [int(hr[0:-1]) for hr in hypo1_simulation_results_df.index]
hypo1_simulation_results_df['experimental_condition'] = ["_".join(s.split("_")[0:-1]) for s in hypo1_simulation_results_df['experimental_condition']]
hypo1_simulation_results_df.to_csv("../output/tables/hypothesis1_invasiveness_random_seeds.csv")
